...
### State Pattern: Timer difficulty toggling


![statePatternDifficultyToggle](images/statePatternDifficultyToggle.png)
- The State pattern is used to show which difficulty level is currently selected when the user clicks on the respective difficulty buttons. There is a DifficultyStateManager along with the respective difficulty states (EasyDifficultyState, MediumDifficultyState, and HardDifficultyState). The *state transitions* are essentially when the user clicks on any difficulty level. In each of the difficulty toggle buttons (EasyDifficultySelectButton, MediumDifficultySelectButton, and HardDifficultySelectButton), the transparency of the text is toggled between fully opaque and half opaque. Each of these button objects access the DifficultyStateManager's current difficulty state (curDifficulty) through the Chessboard singleton and change state accordingly.

### Decorator Pattern: Timer display


![decoratorPattern](images/decoratorPattern.png)
- The Decorator pattern is implemented in a functional programming manner for efficiency. There are two anonymous functions passed into the TimerActor object inside of Chessboard. These anonymous functions handle converting the *number of seconds* into minutes and seconds displays respectively. The display function inside of TimerActor combines the results of these anonymous decorator functions and displays the result.
...
### Singleton - Factory - Observer Pattern: Promotional Buttons 
![richard-promotional-buttons](images/promotion.png)
- When the chessboard is first initalized it calls upon the Singleton "promotionObserver" to deliver it a dictionary of promotional buttons. This dictionary contains the pointers to all of the buttons to be displayed. Each button is a child of the "promotionButton" which the creator in the factory method. These child buttons contain the variation in icon and piece spawn that will be created once the pawn reaches the end. Once these buttons are created they will be observing the promotionObserver for any pawns that are promoted. The sequence of events for promotion is as follows: 
  1. Pawn reaches the end and invokes the openPromotion method in the promotionObserver singleton.
  2. The promotionObserver will then let each of the instantiated promotionButtons know that the promotion option has opened up and which pawn is ready for promotion.
  3. Each promotionButton is then responsible for changing the color on their respective button.
  4. promotionObserver will also let the chessboard know to display the promotionButtons at the top of the board.
  5. Once one of the buttons is clicked. The promotionButton will take care of swapping out the existing pawn for the new piece that is selected.
  6. If the player is unable to select a new piece in time. Chessboard will automatically invoke the closePromotion in promotionObserver to end any promotion opportunities for that round.  
